...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($password === '' || strlen($password) < 12) {
        ...
        header('Location: ' . route_url('students/reset-password') . '?selector=' . rawurlencode($selector) . '&token=' . rawurlencode($token));
        exit;
    }
    ...
    try {
        ...
        $studentRole = $role !== null && in_array($role, ['student'], true);
        if (!$active || !$studentRole) {
            $_SESSION['error'] = 'Invalid or expired reset link.';
            header('Location: ' . route_url('students/login'));
            exit;
        }
...
<head>
    ...
    <link rel="stylesheet" href="<?= htmlspecialchars(asset_url('students/style.css'), ENT_QUOTES, 'UTF-8'); ?>">
...
<body>
    <div class="student-modal" role="dialog" aria-modal="true" aria-labelledby="forgot-title">
        ...
        <?php if ($page_error): ?>
            <p class="modal-message error"><?= htmlspecialchars((string)$page_error, ENT_QUOTES, 'UTF-8'); ?></p>
        <?php endif; ?>
        <?php if ($page_success): ?>
            <p class="modal-message success"><?= htmlspecialchars((string)$page_success, ENT_QUOTES, 'UTF-8'); ?></p>
        <?php endif; ?>
        <form method="POST" action="<?= route_url('students/reset-password') ?>" novalidate autocomplete="off">
            <?= csrf_input(); ?>
            <input type="hidden" name="selector" value="<?= htmlspecialchars($selector, ENT_QUOTES, 'UTF-8'); ?>">
            <input type="hidden" name="token" value="<?= htmlspecialchars($token, ENT_QUOTES, 'UTF-8'); ?>">
            <input type="password" name="password" required aria-required="true" autocomplete="new-password" placeholder="New password" />
            <input type="password" name="password_confirm" required aria-required="true" autocomplete="new-password" placeholder="Confirm new password" />
