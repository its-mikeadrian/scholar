...
$genericOk = 'If a student account exists for that email, a reset link has been sent.';
...
function rate_limited_for_reset(PDO $pdo, ?int $userId, ?string $ip): bool
{
...
$isStudentRole = $role !== null && in_array($role, ['student'], true);

audit_event($conn, $userId, $email, $role, 'pw_reset_requested', $ipAddr, $ua);

if (!$userId || !$isActive || !$isStudentRole) {
    $_SESSION['success'] = $genericOk;
    header('Location: ' . route_url('students/login'));
    exit;
}

if (rate_limited_for_reset($pdo, $userId, $ipAddr)) {
    $_SESSION['success'] = $genericOk;
    header('Location: ' . route_url('students/login'));
    exit;
}
...
$resetPath = route_url('students/reset-password');
$resetUrl = $host !== ''
...
try {
    ...
    $mail->addAddress($email, (string)($user['username'] ?? 'Student'));
    $mail->Subject = 'Reset your student password';
    $mail->isHTML(true);
    $mail->Body =
    ...
    $mail->AltBody = "Reset your student password:\n$resetUrl\n\nThis link expires in 30 minutes.";
    $mail->send();
